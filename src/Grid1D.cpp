      ...
      if (!currentskip) {
        ...
        *result = Model->Fit();
        ...
        if (i >= 1) {
          ...
          // Grid1D.cpp: In instantiation of
          // 'std::vector<std::unique_ptr<FitResult<T> > > Grid1D<T>::Fit()
          // [with T = arma::Mat<double>]': Grid1D.cpp:234:16:   required from
          // here Grid1D.cpp:194:26: warning: variable 'samesupp' set but not
          // used [-Wunused-but-set-variable]
          //     194 |                     bool samesupp = false;
          //         |                          ^~~~~~~~
          // Grid1D.cpp: In instantiation of
          // 'std::vector<std::unique_ptr<FitResult<T> > > Grid1D<T>::Fit()
          // [with T = arma::SpMat<double>]': Grid1D.cpp:235:16:   required from
          // here Grid1D.cpp:194:26: warning: variable 'samesupp' set but not
          // used [-Wunused-but-set-variable]
          // bool samesupp = false;
