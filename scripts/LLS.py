    ...
    for pair in sorted_keys:
        ...
        if count % binsize == 0:
            ...
            benchdata[count] = {"cumLLS": lls, "TP": TP, "FP": FP, "MeanBinStatistics": np.mean(
                binstats), "GeneCoverage": len(already), "BinLLS": binlls}
...
# row : gene, column : sample, no NA values, tab seperated
edges_all = read_edgelist(corrnetpath)
